...
export class NeuralNetwork {
    ...
    public display2Input1Output(ctx, left: number, top: number, width:number, height:number, axis1low: number, axis2low: number, axis1high: number, axis2high: number, rows:number, columns: number, decimals?: number){
        ...
        for (let i = 0; i < columns; i++){
            for (let j = 0; j < rows; j++){
                ...
                ctx.fillStyle = this.numToColorWhite(val.neurons[0].value);
                ctx.fillRect(x, y, spaceX, spaceY);
